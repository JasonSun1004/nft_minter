...
const startCreating = async () => {
  ...
  const sendToIpfs = async (_imgBuffer) => {
    try {
      const addedImage = await ipfsClient.add(getImageData());
      return ipfsBaseUrl + addedImage.path;
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const fillImageURIs = () => {
    for (let i = 1; i <= metadataList.length; i++) {
      metadataList[i-1].image = imageURIs.get(i);
    }
  };
  ...
  while (editionCount <= editionSize) {
    ...
    imageURIs.set(editionCount, await sendToIpfs());
    editionCount++;
  }
  fillImageURIs();
  writeMetaData(JSON.stringify(metadataList));
  require("child_process").fork("splitMetadata.js");
};
